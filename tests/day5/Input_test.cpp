...
TEST_CASE("Input day 5")
{
    ...
    SECTION("Returns it in a list of 4 instructions")
    {
        // Arrange

        // Act
        const auto& result = sut->getInstructions();

        // Assert
        REQUIRE(result.size() == 4);
    }

    SECTION("First instruction has correct information")
    {
        // Arrange
        ...
        // Assert
        const auto& instruction = *result.begin();

        REQUIRE(instruction->noToMove == 1);
        REQUIRE(instruction->beginStackNumber == 2);
        REQUIRE(instruction->endStackNumber == 1);
    }
}